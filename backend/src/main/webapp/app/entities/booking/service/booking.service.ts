...
type RestOf<T extends IBooking | NewBooking> = Omit<T, 'createdAt' | 'cancelledAt'> & {
...
export class BookingService {
  ...
  protected convertDateFromClient<T extends IBooking | NewBooking | PartialUpdateBooking>(booking: T): RestOf<T> {
    return {
      ...
      cancelledAt: booking.cancelledAt?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restBooking: RestBooking): IBooking {
    return {
      ...
      cancelledAt: restBooking.cancelledAt ? dayjs(restBooking.cancelledAt) : undefined,
    };
  }
