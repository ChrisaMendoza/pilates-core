...
type RestOf<T extends IEvent | NewEvent> = Omit<T, 'startAt' | 'endAt'> & {
  startAt?: string | null;
  endAt?: string | null;
};
...
export class EventService {
  ...
  protected convertDateFromClient<T extends IEvent | NewEvent | PartialUpdateEvent>(event: T): RestOf<T> {
    return {
      ...event,
      startAt: event.startAt?.toJSON() ?? null,
      endAt: event.endAt?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restEvent: RestEvent): IEvent {
    return {
      ...restEvent,
      startAt: restEvent.startAt ? dayjs(restEvent.startAt) : undefined,
      endAt: restEvent.endAt ? dayjs(restEvent.endAt) : undefined,
    };
  }
